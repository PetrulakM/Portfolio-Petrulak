...
function encounterBox() {
  const box = document.querySelectorAll('.encounterScreen');
  const adam  = document.querySelector('.adam2');

  box.forEach((area) => {
    const currentOpacity2 = window.getComputedStyle(area).opacity;
    if (currentOpacity2 === '0.5') {
...
function fade(screen, initOp, finalOp, TimeToFade, time)
{
    if (initOp==0)
    {
    ...
    if (Math.abs(newOp-initOp)&gtMath.abs(finalOp-initOp));
    {
